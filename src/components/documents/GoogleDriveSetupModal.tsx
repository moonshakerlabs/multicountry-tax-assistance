    ...
    async function fetchConfig() {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/google-drive-config`
        );
        ...
        if (data.client_id) {
          ...
          setErrorMessage(isDE
            ? 'Google Drive Integration ist nicht konfiguriert.'
      ...
      case 'complete':
        return (
          <div className="gdrive-setup-content gdrive-setup-success">
            ...
            <p>{isDE ? 'Ordner "WordTaxFiling" wurde in Ihrem Google Drive erstellt.' : '"WordTaxFiling" folder has been created in your Google Drive.'}</p>
          </div>
        );
