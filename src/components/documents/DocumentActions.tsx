...
export default function DocumentActions({ 
  ...
  const handleDownload = async () => {
    ...
    try {
      const { data, error } = await supabase.storage
        .from('user-documents')
        .createSignedUrl(document.file_path, 60); // 1 minute
    ...
    <div className="doc-actions-overlay" onClick={onClose}>
      <div className="doc-actions-modal" onClick={e => e.stopPropagation()}>
        <div className="doc-actions-header">
          <h2 className="doc-actions-title">
            {isDE ? 'Dokumentaktionen' : 'Document Actions'}
          </h2>
        ...
        <div className="doc-actions-content">
          ...
          <div className="doc-edit-section">
            <h3 className="doc-edit-title">
              {isDE ? 'Metadaten bearbeiten' : 'Edit Metadata'}
            </h3>
            <div className="doc-edit-form">
              <div className="doc-edit-field">
                <label className="doc-edit-label">
                  {isDE ? 'Steuerjahr' : 'Tax Year'}
                </label>
                <select
                  value={taxYear}
              ...
              <div className="doc-edit-field">
                <label className="doc-edit-label">
                  {isDE ? 'Hauptkategorie' : 'Main Category'}
                </label>
                <select
                  value={mainCategory}
                  onChange={(e) => {
              ...
              {mainCategory && (
                <div className="doc-edit-field">
                  <label className="doc-edit-label">
                    {isDE ? 'Unterkategorie' : 'Sub Category'}
                  </label>
                  <select
                    value={subCategory}
