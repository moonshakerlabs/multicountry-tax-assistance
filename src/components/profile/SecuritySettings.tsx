...
import { Shield, Clock, Key, HelpCircle, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function SecuritySettings({ onTimeoutChange }: { onTimeoutChange?: (minutes: number) => void }) {
  ...
  const handleSaveTimeout = async () => {
    ...
    try {
      const { error } = await supabase
        .from('user_security_settings')
        .upsert({ user_id: user.id, session_timeout_minutes: timeoutMinutes }, { onConflict: 'user_id' });
  ...
  const handleSaveSecurityQuestions = async (e: React.FormEvent) => {
    ...
    if (!q1 || !a1 || !q2 || !a2 || !q3 || !a3) {
      toast.error('Please fill in all 3 security questions and answers');
      return;
    }
    if (a1 === '••••••••' || a2 === '••••••••' || a3 === '••••••••') {
      toast.error('Please enter actual answers (click the fields to update them)');
      return;
    }
    ...
    try {
      const { error } = await supabase
        ...
        .upsert({
          ...
          security_answer_3: a3.toLowerCase().trim(),
        }, { onConflict: 'user_id' });
      if (error) throw error;
      ...
      <section className="rounded-xl border border-border bg-card p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              2FA is <span className="font-medium text-primary">enabled</span> for your account. An email verification code is required every time you sign in.
            </p>
          </div>
        </div>
      </section>
